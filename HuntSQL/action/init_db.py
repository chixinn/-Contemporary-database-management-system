...
class Players(base):
    ...
    pname = Column('pname',String(64), nullable=False, unique=True,primary_key=True) # 用户名和密码都不可为空，且用户名是唯一索引
    pid = Column('pid',Integer, primary_key=True,nullable=False,autoincrement=True,unique=True)# 设定主键
    #在pname上添加索引增加用户体验
...
class Treasures(base):
    ...
    # tid = Column(Integer, primary_key=True, autoincrement=True,unique=True)
    tid = Column('tid',Integer, primary_key=True,nullable=False,autoincrement=True,unique=True)
...
class Box(base):
    # 叙述一下为什么不在box的pid和tid上建立索引
    #跟宝物数量比起来 肯定是玩家的数量多
    __tablename__ = 'box'
    bid = Column(Integer,autoincrement=True,primary_key=True,unique=True)
    # pid = Column(Integer, ForeignKey('players.pid'),primary_key=True,unique=True)
    # tid = Column(Integer, ForeignKey('treasures.tid'),unique=True)
    # bid = Column(Integer, primary_key=True)
    pid = Column(Integer, ForeignKey('players.pid'),primary_key=True)
...
class Markets(base):
    ...
    mid = Column('mid',Integer, autoincrement=True,primary_key=True)
    sid=Column('sid',Integer,ForeignKey('players.pid'),primary_key=True)
    tid = Column('tid',Integer,ForeignKey('treasures.tid'))
   #因为treasure会有市场上好多玩家好多条 就不嵌套了 但takeaway不同
...
class Takeaway(base):
    ...
    aid = Column(Integer,autoincrement=True, unique=True,primary_key=True)#通过这种主键保证删除的唯一性,如果不设置不主键会爆粗！！！！
    pid=Column(Integer, ForeignKey('players.pid'),primary_key=True)#只能在player上创建主键
    tname=Column(String(64), ForeignKey('treasures.tname'),nullable=True)#可以是空的 不可以创建主键
    tid=Column(Integer,ForeignKey('treasures.tid'),nullable=True,primary_key=True)#可以是空的 不可以创建主键
    tlevel=Column(Integer,nullable=True)
    ttype=Column(String(32),nullable=True)
...
def init_user_takeaway(engine):
    session.add_all(
        [
        Takeaway(pid=1,tname='rope',tid=83,tlevel=5,ttype='working'),
        Takeaway(pid=1,tname='diya',tid=82,tlevel=5,ttype='fortune'),
        Takeaway(pid=1,tname='diamond',tid=81,tlevel=10,ttype='fortune'),

        Takeaway(pid=2,tname='rope',tid=83,tlevel=5,ttype='working'),
        Takeaway(pid=2,tname='diya',tid=82,tlevel=5,ttype='fortune'),
        Takeaway(pid=2,tname='diamond',tid=81,tlevel=10,ttype='fortune'),
        Takeaway(pid=3,tname='rope',tid=83,tlevel=5,ttype='working'),
        Takeaway(pid=3,tname='diya',tid=82,tlevel=5,ttype='fortune'),
        Takeaway(pid=3,tname='diamond',tid=81,tlevel=10,ttype='fortune')
        ]
    )
    session.commit() 
